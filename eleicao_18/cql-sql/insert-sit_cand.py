...
df_sit_cand = df_geral.select("cd_situacao_candidatura", "ds_situacao_candidatura").distinct()
# df_sit_cand.show(20)

# Inserindo no banco MySQL com JDBC
df_sit_cand.write.format("jdbc") \
    .option("url", f"jdbc:mysql://mysql-app/{db}?user=root&password=root") \
    .mode("append") \
	.option("driver", "com.mysql.cj.jdbc.Driver") \
    .option("dbtable", "sit_cand").save()
...
# df_sit_cand = df_sit_cand.toPandas()
# print(df_sit_cand.head(5))

# # Inserindo o Dataframe na tabela criada.
# df_sit_cand.to_sql(name='sit_cand', con=conection, if_exists="append", index=False)